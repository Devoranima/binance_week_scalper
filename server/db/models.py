from sqlalchemy import String, ForeignKey, DateTime, Table, Column, Float, Boolean, ForeignKeyConstraint
...
class Candle (Base, SerializerMixin):
  ...
  timeframe_type: Mapped[str] = mapped_column(ForeignKey("timeframe.type"), primary_key=True)
...
swing_candle_link = Table(
    ...
    ForeignKeyConstraint(
      ...
      ["candle.tradepair_name", "candle.timeframe_type", "candle.datetime_open"]
    )
)
...
class Swing(Base, SerializerMixin):
  ...
  timeframe_type: Mapped[str] = mapped_column(ForeignKey("timeframe.type"), nullable=False)
